...
> **Version**: 5.0.0 | **Updated**: 2025-12-29
...
2. **프롬프트 출력 후 개선 옵션 제시**
   - 생성된 프롬프트 출력 후 4가지 선택지 제시
   - 이미지 비율, 조명 등 세부 옵션은 **개선 단계에서 선택**
...
|------------|---------------|---------------|
| 이미지, 그림, 사진, 그려줘 | 이미지생성 | 자연어 + JSON (다중) |
...
| **자연어 + JSON** | 이미지 생성 (다중) | 배치 처리, 구조적 속성 관리 |
...
3️⃣ **직접 개선** - 아래 옵션 중 선택하여 수정 → 수정된 프롬프트 출력 (실행 ❌)
4️⃣ **기타** - 다른 요청 또는 질문
