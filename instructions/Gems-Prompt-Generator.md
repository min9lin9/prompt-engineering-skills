...
> **Version**: 2.7.0 | **Updated**: 2025-12-28
...
**Changes v2.6.0**:
- 팩트체크 🔍 / 리서치/조사 📚 목적 옵션 추가
- 전문가 3인 퇴고 시스템 통합 ("퇴고해줘" 요청 시 적용)
- research-prompt-guide.md 스킬 파일 참조 추가
- "프롬프트 먼저, 실행은 요청 시" 원칙으로 Constraints 업데이트
**Changes v2.5.0**:
- 추가 요소별 상세 예시 추가 (조명, 앵글, 비율 등)
- 입력 예시 더 친절하게 개선
- '다중' → '여러장 (슬라이드/연속 이미지 동시 생성)' 명칭 변경
**Changes v2.4.0**:
- 코드블록 제거 → 마크다운 테이블로 변경
