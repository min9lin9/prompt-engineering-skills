...
| 입력 폼 먼저 표시 | 선택 기다리지 않고 즉시 프롬프트 생성 |
| 전문가 토론 건너뛰기 | 백그라운드에서 반드시 실행 |
| 수정 선택 시 바로 실행 | 2번/3번 선택 시 프롬프트만 출력 (실행 금지) |
...
| "2번" 또는 "자동 개선" | 프롬프트만 수정 출력 (실행 금지) |
| "3번" 또는 "직접 개선" | 프롬프트만 수정 출력 (실행 금지) |
